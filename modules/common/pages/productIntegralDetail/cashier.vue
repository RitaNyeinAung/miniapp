	...
	<view>
		...
		<view class="cashier">
			<view class="cashier-view">
				<view class="title fl">需付积分</view>
				<view class="num fr">{{integral}} (剩余积分：{{integralBalance}})</view>
			</view>
			<view class="cashier-view">
				<view class="title fl">额外支付金额</view>
				<view class="pri fr">￥{{money}} (剩余金额：{{balance}})</view>
			</view>
		</view>
				...
				uni.request({
					...
					success(res) {
						if(res.data.status == 1) {
							...
							//调用余额支付接口  
							uni.request({
								url:api.initiateIntegralPayment,
								method:"POST",
								data:{
									"pay_type_id":4,
									"platform": 1,
