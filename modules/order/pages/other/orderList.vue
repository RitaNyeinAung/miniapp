<template>
	<view>
		...
		<view class="order-wrap">
            ...
            <image class="fl" v-if="text.pic_url" :src="config.image_url+text.pic_url" @click="checkOrder(item)"></image>
            <image class="fl" v-else src="@/static/images/common/img_bad.jpeg" @click="checkOrder(item)"></image>
					...
					<view class="operation clearfix">
						...
						<view class="btn fr repeat" @click="checkOrder(item)"
								v-if="item.order_status == 4 || item.order_status == -1" v-show="item.order_state != '1'">
							再次购买
						</view>
...
<script>
	...
	export default {
		...
		onLoad(options){
			...
			uni.setStorageSync('OrderID',this.cateId);
		},
		onReachBottom(){
              ...
              uni.request({
                ...
                success: (res) => {
                  if(res.data.status==1){
                    ...
                    uni.navigateTo({
                      url: 'modules/user/pages/index/components/orderList'
                    })
                  }else{
                    uni.showToast({
                      title:res.data.message
      ...
      checkOrder(item){
        uni.navigateTo({
          url: '/modules/user/pages/index/components/orderDetails?id='+item.id,
          fail(e) {
            console.log(e)
          }
        })
