<template>
	<view class="container">
		...
		<view class="carousel">
			<swiper indicator-dots circular=true duration="4000" :autoplay="true" @change="carouselChange" :current="currentCarouselIndex">
		...
		<view class="flashsale" v-if="productInfo">
			<view class="left" v-if="productInfo.delayed==0">
				商品开放购买中
			</view>
			...
			<view class="right">
				<view class="countdown" v-if="productInfo.delayed>0">
					<view class="b hour">{{productInfo.delayed|toHMS('h')}}</view>
					<view class="b minute">{{productInfo.delayed|toHMS('m')}}</view>
					<view class="b second">{{productInfo.delayed|toHMS('s')}}</view>
					<text style="margin-left:5px">后开始</text>
				</view>
		...
		<view class="introduce-section" v-if="productInfo">
			...
			<view class="price-box">
				<view >				
					<text class="price">{{productInfo.integral}}</text>
					<text class="price-tip">积分 </text>
					<text class="price"> + {{productInfo.money}}</text>
					<text class="price-tip">元</text>
					<text class="coupon-tip" v-if="productInfo.price">实际价</text>
					<text class="m-price" v-if="productInfo.price">{{productInfo.price}}</text>				
				</view>
			</view>
			...
			<view class="bot-row">
				...
				<text>重量 : {{productInfo.weight}}kg</text>
			</view>
		</view>
		...
		<view class="eva-section">
			...
			<view class="content" v-else style="text-align: center;font-size: 10px;color: #C0C4CC;">
				暂无套餐
			</view>
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group" v-if="productInfo.delayed==0">
				
				<button type="primary" class=" action-btn no-border add-cart-btn"  @click="toOrder">我要兑换</button>
				
			</view>
			<view class="action-btn-group disabled" v-else>
				<button type="primary" disabled  class="action-btn disable no-border add-cart-btn">商品未开放购买~</button>
			</view>
...
<script>
	...
	export default{
		...
		async onLoad(options){
			...
			let _this = this;
			uni.request({
				url:api.getIntegralProductInfo+"?id="+id+"&token="+uni.getStorageSync("token"),
				success: (res) => {
					...
					if(res.data.data){
						...
						_this.timer = setInterval(()=>{						
							...
							if(this.productInfo.delayed>0){
								//每秒 减去 一千 毫秒
								this.productInfo.delayed=this.productInfo.delayed-1
		...
		methods:{
			...
			getSpec(){
				let _this = this;
				uni.request({
					url:api.getProductSpec+'?id='+_this.productInfo.goods_id,
					success:(res)=>{
						uni.hideLoading();
						if(res.data.data){
							this.skuProducts = res.data.data.goods;
...
<style lang='scss'>
	...
	.page-bottom{
		...
		.action-btn-group{
			...
			background: linear-gradient(to right, red,#F56C6C);
			&.disabled{
				opacity:0.3;
			}
			position:relative;
