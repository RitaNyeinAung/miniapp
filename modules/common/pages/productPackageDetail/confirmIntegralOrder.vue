			...
			<view class="mark-wrap">
						...
						<view class="clearfixli" id="store" style="display:flex">
							...
							<view class="pull-right fl">
								...
								<view class="price-wrap clearfix">
									<text class="fl price">积分<text class="span">{{goods_data.integral||""}}</text></text>
									<text class="number fr">X{{goods_data.goods_num||1}}</text>
								</view>
				...
				<view class="price-set-wrap" style="padding-left: 5px;">
					<view class="total price">
						...
						<text class="fr" v-if="goods_data.integral&&routnum">￥{{((goods_data.integral * routnum).toFixed(2))||0}}</text>
					</view>
					<view class="total price">
						<text class="fl">额外支付金额</text>
						<text class="fr" v-if="goods_data.money&&routnum">￥{{((goods_data.money * routnum).toFixed(2))||0}}</text>
					</view>
					...
					<view class="freight price" v-else-if="status == 0">
						...
						<text v-if="freight == 0" class="fr">&nbsp;&nbsp;卖家包邮</text>
				...
				<view class="footer-wrap" >
					<view class="money">
						...
						<text  style="padding-left: 5px;">
							<text v-if="goods_data.integral&&routnum">{{((goods_data.integral*routnum)).toFixed(2)}}</text>
							<text v-else>0</text>
						</text>
					</view>	
					<view class="money">
						...
						<text  style="padding-left: 5px;">
							<text v-if="goods_data.money&&routnum">￥{{((goods_data.money*routnum)).toFixed(2)}}</text>
	...
	export default {
		...
		methods:{
			...
			getData(){
				...
				let _this = this;
				uni.request({
					url:api.getIntegralProductDetail,
					method:"GET",
					data:{
						token:uni.getStorageSync("token")||"-",
						id:this.routid,
			...
			toCashierAll() {
				...
				url = api.generateIntegralProductOrder
