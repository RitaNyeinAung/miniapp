<template>
	<view class="container">
		...
		<view class="flashsale" v-if="productInfo">
			<view class="left" v-if="productInfo.countdown>0">
				活动进行中
			</view>
			...
			<view class="right">
				<view class="countdown" v-if="productInfo.countdown>0">
					<view class="b hour">{{productInfo.countdown|toHMS('h')}}</view>
					<view class="b minute">{{productInfo.countdown|toHMS('m')}}</view>
					<view class="b second">{{productInfo.countdown|toHMS('s')}}</view>
				</view>
				<view class="sold">
					<view class="bar">
						<view class="innerbar" :style="{width:(productInfo.buy_num/productInfo.activity_num)*100+'%'}"></view>
					</view>
					<text>已售 {{productInfo.buy_num}}</text>
				</view>
			</view>
		</view>
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group" v-if="productInfo.countdown>0">
				
				<button type="primary" class=" action-btn no-border add-cart-btn"  @click="toOrder">立马抢购</button>
				
			</view>
			<view class="action-btn-group disabled" v-else>
				<button type="primary" disabled  class="action-btn disable no-border add-cart-btn">活动已结束~</button>
			</view>
...
<script>
	...
	export default{
		...
		async onLoad(options){
			...
			let _this = this;
			uni.request({
				url:api.getSecKillProductDetail+'?goods_id='+goods_id+"&id="+id,
				success: (res) => {
					...
					if(res.data.data){
						...
						this.images = res.data.data.list.images;
						this.productInfo = res.data.data.list;
						_this.timer = setInterval(()=>{						
						
							if(this.productInfo.countdown>0){
								//每秒 减去 一千 毫秒
								this.productInfo.countdown=this.productInfo.countdown-1
			...
			uni.request({
				url:api.getProductSpec+'?id='+goods_id,
				success:(res)=>{
					uni.hideLoading();
					if(res.data.data){
						this.skuProducts = res.data.data.goods;
