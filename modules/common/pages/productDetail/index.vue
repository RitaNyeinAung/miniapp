	...
	<view class="container">
			...
			<swiper indicator-dots circular=true duration="400" :autoplay="true" @change="carouselChange" :current="currentCarouselIndex">
				<swiper-item class="swiper-item" v-for="(item,index) in images" :key="index">
					<view class="image-wrapper">
						<image v-if="item.pic_url" :src="config.image_url + item.pic_url" class="loaded" mode="aspectFill"></image>
					</view>
				</swiper-item>
		...
		<view class="eva-section">
			...
			<template v-if="commentList.length>0">
				<view class="eva-box" >
					<image class="portrait" v-if="commentList[0].user_header" :src="config.image_url+commentList[0].user_header" mode="aspectFill"></image>
					<view class="right">
		...
		<view class="eva-section">
			<view class="e-header">
				<view class="tit">
					<view class="image">
						<image v-if="shopInfo.store_logo" :src="config.image_url+shopInfo.store_logo"></image>
            <image v-else src="@/static/images/common/logo3.png"></image>
					</view>
					<view class="title">
		...
		<view class="page-bottom">
			...
			<view class="p-b-btn" :class="{active: favorite}" @click="addToCollection">
				<text class="yticon icon-shoucang"></text>
				<text>收藏</text>
			</view>
			
			<view class="action-btn-group">
				<button type="primary" class=" action-btn no-border buy-now-btn" @click="toOrder">立即购买</button>
			...
			<view class="layer" @click.stop="stopPrevent">
				...
				<view class="middle attr-content">
					<view class="a-t" v-if="!noProductMatch">
						<image v-if="tempSelectedSkuProduct.pic_url" :src="config.image_url+tempSelectedSkuProduct.pic_url"></image>
						...
						<view class="right">
							...
							<text class="stock">库存：{{tempSelectedSkuProduct.stock}}</text>
							<view class="selected">
								已选：
								<text class="selected-text" >
...
<script>
	...
	export default{
		...
		async onLoad(options){
			...
			let _this = this;
			uni.request({
				url:api.getProductInfoById+'?id='+id,
				success: (res) => {
					uni.hideLoading();
					if(res.data.data){
						this.images = res.data.data.images;
		...
		methods:{
			...
			addToCollection(){
				let type = this.favorite?2:1;
				let goods_id = this.productInfo.id;
				uni.request({
