	...
	<view>
		<view class="order-wrap">
			...
			<view class="mark-wrap">
				<view v-if="routid&&routnum&&!routids" >
					<!-- 立即购买 -->
					<view class="store" v-if="store&&goods_data">
						...
						<view class="clearfixli li" id="store">
							...
							<view class="pull-right fl">
								<view class="text">{{goods_data.title||""}}</view>
								<view class="price-wrap clearfix">
									<text class="fl price">￥<text class="span">{{goods_data.price_member||""}}</text></text>
				...
				<view v-else-if="!routid&&!routnum&&routids">
					<!-- 购物车购买 -->
					
					<view class="store" v-for="store in cartData.store">
						<view class="header li"><image :src="config.image_url + store.store_logo"></image><text class="span">{{store.shop_name}}</text></view>
						<view class="clearfixli li" id="store" v-for="item in cartData.shop_goods_info[store.id]">
							<image v-if="item.pic_url" :src="config.image_url + item.pic_url " class="fl"></image>
							<image v-else :src="apiLink + '/goods/2019-08-16/5d564e9785339.jpg'" class="fl"></image>
							<view class="pull-right fl">
								<view class="text">{{item.title}}</view>
								<view class="price-wrap clearfix">
									<text class="number fr">数量 ：{{item.goods_num}}</text>
									<text class="fl price">￥<text class="span">{{item.price_member}}</text></text>
				...
				<view class="dist-wrap">
					...
					<view class="hd " @click="toggleCoupon" v-if="routid != 3">
						<view class="title fl">优惠券</view>
						<view>
							<view class="busi">{{coupon.name?coupon.name:"请选择优惠券"}}</view>
				...
				<view class="price-set-wrap" v-if="routid&&routnum&&!routids" style="padding-left: 5px;">
					<view class="total price">
						<text class="fl">税前商品总额</text>
						<text class="fr" v-if="goods_data.price_member&&goods_data.goods_num">￥{{((goods_data.price_member * goods_data.goods_num).toFixed(2))||0}}</text>
					</view>
				...
				<view class="price-set-wrap" v-if="!routid&&!routnum&&routids&&cartData" style="padding-left: 5px;">
					<view class="total price">
						<text class="fl">税前商品总额</text>
						<text class="fr" v-if="cartData">￥{{cartData.total_price||0}}</text>
					</view>
					<view class="freight price" v-if="cartData">
						<text class="fl" v-if="cartData.discount&&cartData.discount.memberLevel">{{cartData.discount.memberLevel}}</text>
						<text class="fr" v-if="cartData.discount">&nbsp;&nbsp;{{Number(cartData.discount.discount)/10}} 折优惠</text>
					</view>
					<view class="freight price" v-if="money == -1">
						<text class="fl">配送共计</text>
						<text class="fr">+&nbsp;&nbsp;免配送费</text>
				...
				<view class="footer-wrap" >
					<view class="money">
						...
						<text v-if="!routid&&!routnum&&routids&&cartData" style="padding-left: 5px;">
							<text>￥{{(((cartData.total_price*cartData.discount.discount/100)-coupon.money)<0?0.00:(cartData.total_price*cartData.discount.discount/100)-coupon.money)||0}}</text>
						</text>
					</view>	
	...
	export default {
		...
		methods:{
			...
			getData(){
				...
				if(this.routid&&this.routnum&&!this.routids){
					...
					uni.request({
						...
						if(res.data.status == 1){
							...
							let listKey = Object.keys(_this.invoiceGroup);
							for (const i in listKey) {
								if (_this.invoiceGroup[listKey[i]]) {
										_this.show[i]= _this.invoiceGroup[listKey[i]].translate;
									}
								}
							}else{
								let invoiceData = {};
				...
				else if(!this.routid&&!this.routnum&&this.routids){
					let _this = this;
					uni.request({
						url:api.userBuyCartGoods,
						method:"POST",
						data:{
							token:uni.getStorageSync("token")||"-",
							cartId:this.routids
			...
			toCashierAll() {
				...
				if(!this.routid&&!this.routnum&&this.routids){
					let total = ((this.cartData.total_price*this.cartData.discount.discount/100)-this.coupon.money)<0?0.00:(this.cartData.total_price*this.cartData.discount.discount/100)-this.coupon.money
					url = api.orderBeginFromCart
					data.token=uni.getStorageSync('token')
