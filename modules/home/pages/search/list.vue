			...
			<view class="cate-content" @click.stop.prevent="stopPrevent" @touchmove.stop.prevent="stopPrevent">
				<scroll-view scroll-y class="cate-list">
					...
					<view class="btn">
						...
						<u-button type="error" :custom-style="{backgroundColor:config.main_color,color:'white',borderRadius:0,height:'100rpx'}" @click="()=>{loadData('refresh',1);cateMaskState=0}">确认</u-button>
					</view>
				</scroll-view>
		...
		methods: {	
			selectedItem(){
				...
				uni.request({
					url : url,
					success: (res) => {
						_this.brandlist= res.data.data.data.brand;
						_this.classify = res.data.data.data.classification;
						_this.attributes = res.data.data.data.attr;
					},
					fail: (res)=>{
						console.log(res);
					}
